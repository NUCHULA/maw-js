...
const $ = { charColor: "prevCharColorBuffer", char: "prevCharBuffer", cellColor: "prevCellColorBuffer" }, k = { charColor: "_charColor", char: "_char", cellColor: "_cellColor" }, ue = "src", fe = "coord", pe = "color", de = "combine", me = "combineCoord", ee = /* @__PURE__ */ new Set(["combine", "combineCoord"]), Q = /* @__PURE__ */ new Set(["coord", "combineCoord"]), he = { [ue]: { returnType: "vec4", args: [{ type: "vec2", name: "_st" }] }, [fe]: { returnType: "vec2", args: [{ type: "vec2", name: "_st" }] }, [pe]: { returnType: "vec4", args: [{ type: "vec4", name: "_c0" }] }, [de]: { returnType: "vec4", args: [{ type: "vec4", name: "_c0" }, { type: "vec4", name: "_c1" }] }, [me]: { returnType: "vec2", args: [{ type: "vec2", name: "_st" }, { type: "vec4", name: "_c0" }] } };
function ye(t) {
  const e = he[t.type], r = [...e.args, ...t.inputs.map((n) => ({ type: n.type, name: n.name }))].map((n) => `${n.type} ${n.name}`).join(", "), o = `
${e.returnType} ${t.name}(${r}) {
${t.glsl}
}`;
  return { ...t, glslFunction: o };
}
class ge {
  constructor() {
    __publicField(this, "_transforms", /* @__PURE__ */ new Map());
    __publicField(this, "_processedCache", /* @__PURE__ */ new Map());
  }
  register(e) {
  ...
  getProcessed(e) {
    ...
    if (!r) {
      const o = this._transforms.get(e);
      o && (r = ye(o), this._processedCache.set(e, r));
    }
    return r;
  }
  has(e) {
...
const R = new ge(), W = /* @__PURE__ */ new Set(["src"]);
class _e {
  constructor() {
    __publicField(this, "_generatedFunctions", {});
    __publicField(this, "_synthSourceClass", null);
  }
  ...
  injectMethods(e) {
    const r = R.getAll();
    for (const o of r) this._injectMethod(e, o);
  }
  _injectMethod(e, r) {
    const o = this._synthSourceClass, { name: n, inputs: a, type: i } = r, u = e;
    if (ee.has(i)) u[n] = function(p, ...d) {
      let s = p;
      if (o && !(p instanceof o)) {
        const c = new o(), l = typeof p == "number" ? [p, p, p, null] : [p, null, null, null];
        c.addTransform("solid", l), s = c;
      }
      return this.addCombineTransform(n, s, z(a, d));
    };
    else {
      const p = this;
      u[n] = function(...d) {
        return d = p._expandColorArgs(n, d), this.addTransform(n, z(a, d));
      };
    }
  }
  _expandColorArgs(e, r) {
    if ((e === "solid" || e === "color") && r.length === 1 && typeof r[0] == "number") {
      const o = r[0];
      return [o, o, o];
    }
    return r;
  ...
  generateStandaloneFunctions() {
    ...
    const e = {}, r = R.getAll(), o = this._synthSourceClass;
    for (const n of r) if (W.has(n.type)) {
      const { name: a, inputs: i } = n;
      e[a] = (...u) => {
        const p = new o();
        return u = this._expandColorArgs(a, u), p.addTransform(a, z(i, u));
      };
    }
  ...
  addTransform(e, r) {
    if (R.register(e), r && this._injectMethod(r, e), W.has(e.type) && this._synthSourceClass) {
      const o = this._synthSourceClass, { name: n, inputs: a } = e;
      this._generatedFunctions[n] = (...i) => {
        const u = new o();
        return i = this._expandColorArgs(n, i), u.addTransform(n, z(a, i));
      };
    }
  }
}
function z(t, e) {
  return t.map((r, o) => e[o] ?? r.default);
}
const N = new _e(), ve = { name: "osc", type: "src", inputs: [{ name: "frequency", type: "float", default: 60 }, { name: "sync", type: "float", default: 0.1 }, { name: "offset", type: "float", default: 0 }], glsl: `
	vec2 st = _st;
...
`, description: "Sample from a TextmodeSource (image/video). Context-aware: the actual sampler uniform is determined at compile time based on the source reference." }, Pe = [ve, xe, Ce, be, Se, we, $e, Me, Te, Fe];
function te(t) {
...
const ke = { name: "rotate", type: "coord", inputs: [{ name: "angle", type: "float", default: 10 }, { name: "speed", type: "float", default: 0 }], glsl: `
	vec2 xy = _st - vec2(0.5);
	float ang = angle + speed * time;
	xy = mat2(cos(ang), -sin(ang), sin(ang), cos(ang)) * xy;
	xy += 0.5;
	return xy;
`, description: "Rotate coordinates" }, Ie = { name: "scale", type: "coord", inputs: [{ name: "amount", type: "float", default: 1.5 }, { name: "xMult", type: "float", default: 1 }, { name: "yMult", type: "float", default: 1 }, { name: "offsetX", type: "float", default: 0.5 }, { name: "offsetY", type: "float", default: 0.5 }], glsl: `
...
`, description: "Convert to polar coordinates" }, Xe = { name: "twirl", type: "coord", inputs: [{ name: "amount", type: "float", default: 2 }, { name: "radius", type: "float", default: 0.5 }, { name: "centerX", type: "float", default: 0.5 }, { name: "centerY", type: "float", default: 0.5 }], glsl: `
...
`, description: "Mirror coordinates across X and/or Y axes" }, Ne = { name: "shear", type: "coord", inputs: [{ name: "x", type: "float", default: 0 }, { name: "y", type: "float", default: 0 }, { name: "centerX", type: "float", default: 0.5 }, { name: "centerY", type: "float", default: 0.5 }], glsl: `
	vec2 center = vec2(centerX, centerY);
	vec2 p = _st - center;
	p = vec2(p.x + y * p.y, p.y + x * p.x);
	return p + center;
`, description: "Shear coordinates along X and Y" }, qe = { name: "barrel", type: "coord", inputs: [{ name: "amount", type: "float", default: 0.5 }, { name: "centerX", type: "float", default: 0.5 }, { name: "centerY", type: "float", default: 0.5 }], glsl: `
	vec2 center = vec2(centerX, centerY);
	vec2 p = _st - center;
	float r2 = dot(p, p);
	p *= 1.0 + amount * r2;
	return p + center;
`, description: "Barrel distortion (bulge outward)" }, Ee = { name: "pinch", type: "coord", inputs: [{ name: "amount", type: "float", default: 0.5 }, { name: "centerX", type: "float", default: 0.5 }, { name: "centerY", type: "float", default: 0.5 }], glsl: `
	vec2 center = vec2(centerX, centerY);
	vec2 p = _st - center;
	float r2 = dot(p, p);
	p *= 1.0 / (1.0 + amount * r2);
...
`, description: "Pinch distortion (pull inward)" }, Ge = { name: "fisheye", type: "coord", inputs: [{ name: "amount", type: "float", default: 1 }, { name: "centerX", type: "float", default: 0.5 }, { name: "centerY", type: "float", default: 0.5 }], glsl: `
	...
	return p + center;
`, description: "Fisheye lens distortion" }, He = [ke, Ie, Ae, Le, Ue, Re, ze, De, je, Ve, Ye, Xe, Be, Oe, Ne, qe, Ee, Ge], Ke = [{ name: "scale", type: "float", default: 1 }, { name: "offset", type: "float", default: 0 }];
function G(t) {
...
`, description: "Shift hue" }, tt = { name: "colorama", type: "color", inputs: [{ name: "amount", type: "float", default: 5e-3 }], glsl: `
	...
	c = fract(c);
	return vec4(c, _c0.a);
`, description: "Color cycle effect" }, rt = { name: "posterize", type: "color", inputs: [{ name: "bins", type: "float", default: 3 }, { name: "gamma", type: "float", default: 0.6 }], glsl: `
...
`, description: "Posterize colors" }, ot = { name: "luma", type: "color", inputs: [{ name: "threshold", type: "float", default: 0.5 }, { name: "tolerance", type: "float", default: 0.1 }], glsl: `
	float a = _smoothThreshold(_luminance(_c0.rgb), threshold, tolerance);
	return vec4(_c0.rgb * a, a);
`, description: "Luma key" }, nt = { name: "thresh", type: "color", inputs: [{ name: "threshold", type: "float", default: 0.5 }, { name: "tolerance", type: "float", default: 0.04 }], glsl: `
...
${M}
`, description: "Darken blend with another source" }, kt = [ht, yt, gt, _t, vt, xt, Ct, bt, St, wt, $t, Mt, Tt, Ft, Pt];
function oe(t) {
  const e = t === "x" ? "scrollX" : "scrollY";
...
const It = { name: "modulate", type: "combineCoord", inputs: [{ name: "amount", type: "float", default: 0.1 }], glsl: `
...
`, description: "Modulate kaleidoscope with another source" }, zt = oe("x"), Dt = oe("y"), jt = { name: "modulateRepeat", type: "combineCoord", inputs: [{ name: "repeatX", type: "float", default: 3 }, { name: "repeatY", type: "float", default: 3 }, { name: "offsetX", type: "float", default: 0.5 }, { name: "offsetY", type: "float", default: 0.5 }], glsl: `
	vec2 st = _st * vec2(repeatX, repeatY);
...
`, description: "Modulate coordinates based on hue differences" }, Bt = [It, At, Lt, Ut, Rt, zt, Dt, jt, Vt, Yt, Xt], Ot = [...Pe, ...He, ...mt, ...kt, ...Bt];
class L {
  constructor(e) {
...
class v {
  constructor(e) {
    ...
    __publicField(this, "_charMapping");
    __publicField(this, "_nestedSources");
    __publicField(this, "_externalLayerRefs");
    __publicField(this, "_charColorSource");
    __publicField(this, "_cellColorSource");
    __publicField(this, "_charSource");
    __publicField(this, "_textmodeSourceRefs");
  ...
  addTransform(e, r) {
    const o = { name: e, userArgs: r };
    return this._chain.push(o), this;
  }
  addCombineTransform(e, r, o) {
    const n = this._chain.length;
    return this._nestedSources.set(n, r), this.addTransform(e, o);
  }
  ...
  addTextmodeSourceRef(e) {
    const r = this._chain.length;
    return this._textmodeSourceRefs.set(r, e), this.addTransform("srcTexture", []);
  }
  charMap(e) {
    if (e.length === 0) return this._charMapping = void 0, this;
    const r = Array.from(e), o = [];
    for (const n of r) o.push(n.codePointAt(0) ?? 32);
    return this._charMapping = { chars: e, indices: o }, this;
  }
  _ensureSource(e, r, o, n) {
    if (e instanceof v) return e;
    const a = new v(), i = typeof e == "number" && r === void 0 && o === void 0 && n === void 0 ? [e, e, e, null] : [e, r, o, n].map((u) => u === void 0 ? null : u);
    return a.addTransform("solid", i), a;
  }
  charColor(e, r, o, n) {
    return this._charColorSource = this._ensureSource(e, r, o, n), this;
  }
  char(e) {
    return this._charSource = e, this;
  }
  cellColor(e, r, o, n) {
    return this._cellColorSource = this._ensureSource(e, r, o, n), this;
  }
  paint(e, r, o, n) {
    const a = this._ensureSource(e, r, o, n);
  ...
  clone() {
    ...
    for (const [n, a] of this._externalLayerRefs) r.set(n, { ...a });
    const o = /* @__PURE__ */ new Map();
    for (const [n, a] of this._textmodeSourceRefs) o.set(n, { ...a });
    return new v({ chain: L.from(this._chain.transforms), charMapping: this._charMapping, charColorSource: (_a = this._charColorSource) == null ? void 0 : _a.clone(), cellColorSource: (_b = this._cellColorSource) == null ? void 0 : _b.clone(), charSource: (_c = this._charSource) == null ? void 0 : _c.clone(), nestedSources: e, externalLayerRefs: r, textmodeSourceRefs: o });
  }
  get transforms() {
...
function Nt(t, e, r, o, n) {
  return e === r ? (o + n) / 2 : (t - e) * (n - o) / (r - e) + o;
}
function qt() {
  "fast" in Array.prototype || (Object.defineProperty(Array.prototype, "fast", { value: function(t = 1) {
    return this._speed = t, this;
  }, writable: true, configurable: true, enumerable: false }), Object.defineProperty(Array.prototype, "smooth", { value: function(t = 1) {
    return this._smooth = t, this;
  }, writable: true, configurable: true, enumerable: false }), Object.defineProperty(Array.prototype, "ease", { value: function(t = "linear") {
    return typeof t == "function" ? (this._smooth = 1, this._ease = t) : q[t] && (this._smooth = 1, this._ease = q[t]), this;
  }, writable: true, configurable: true, enumerable: false }), Object.defineProperty(Array.prototype, "offset", { value: function(t = 0.5) {
    return this._offset = t % 1, this;
  }, writable: true, configurable: true, enumerable: false }), Object.defineProperty(Array.prototype, "fit", { value: function(t = 0, e = 1) {
    const r = Math.min(...this), o = Math.max(...this), n = this.map((a) => Nt(a, r, o, t, e));
    return n._speed = this._speed, n._smooth = this._smooth, n._ease = this._ease, n._offset = this._offset, n;
  }, writable: true, configurable: true, enumerable: false }));
}
function Et(t, e) {
  const r = t._speed ?? 1, o = t._smooth ?? 0;
  let n = e.time * r * (e.bpm / 60) + (t._offset ?? 0);
  if (o !== 0) {
    const a = t._ease ?? q.linear, i = n - o / 2, u = Math.floor(i), p = J(u, t.length), d = (p + 1) % t.length, s = t[p], c = t[d];
    return a(Math.min((i - u) / o, 1)) * (c - s) + s;
  }
  return t[Math.floor(J(n, t.length))];
}
function Gt(t) {
  return Array.isArray(t) && t.length > 0 && typeof t[0] == "number";
}
qt(), R.registerMany(Ot), N.setSynthSourceClass(v), N.injectMethods(v.prototype);
const w = N.generateStandaloneFunctions(), b = "textmode.synth.js";
function E(t) {
  return t === "char" || t === "cellColor" ? t : "charColor";
}
function ae(t, e) {
  const r = E(e);
  r === "char" ? t.usesChar = true : r === "cellColor" ? t.usesCellColor = true : t.usesCharColor = true;
}
class Ht {
  constructor() {
    __publicField(this, "_usage", { usesChar: false, usesCharColor: false, usesCellColor: false });
  }
  trackUsage(e) {
    ae(this._usage, e);
  }
  reset() {
    this._usage.usesChar = false, this._usage.usesCharColor = false, this._usage.usesCellColor = false;
  }
  getUsage() {
...
class Kt {
  constructor() {
    __publicField(this, "_externalLayers", /* @__PURE__ */ new Map());
    __publicField(this, "_counter", 0);
    __publicField(this, "_layerIdToPrefix", /* @__PURE__ */ new Map());
  }
  getPrefix(e) {
  ...
  trackUsage(e, r) {
    const o = this.getPrefix(e.layerId);
    let n = this._externalLayers.get(e.layerId);
    n || (n = { layerId: e.layerId, uniformPrefix: o, usesChar: false, usesCharColor: false, usesCellColor: false }, this._externalLayers.set(e.layerId, n)), ae(n, r);
  }
  hasLayer(e) {
...
class Qt {
  constructor() {
    __publicField(this, "_sources", /* @__PURE__ */ new Map());
    __publicField(this, "_counter", 0);
  }
  reset() {
  ...
  trackUsage(e, r) {
    let o = this._sources.get(e.sourceId);
    switch (o || (o = { sourceId: e.sourceId, uniformName: "u_tms" + this._counter++, usesChar: false, usesCharColor: false, usesCellColor: false, sourceRef: e }, this._sources.set(e.sourceId, o)), r) {
      case "char":
        o.usesChar = true;
        break;
      case "charColor":
      case "main":
        o.usesCharColor = true;
        break;
      case "cellColor":
        o.usesCellColor = true;
    }
  }
  getSources() {
  ...
  getUniformName(e) {
    var _a;
    return ((_a = this._sources.get(e)) == null ? void 0 : _a.uniformName) ?? "u_tms0";
  }
  hasAnySources() {
...
class Wt {
  getContextAwareGlslFunction(e, r, o, n, a, i, u) {
    return r === "srcTexture" && a && u ? this._generateTextmodeSourceFunction(a, o, u) : r !== "src" ? e.glslFunction : n && i ? this._generateExternalSrcFunction(n, o, i) : this._generateSelfFeedbackSrcFunction(o);
  }
  getFunctionName(e, r, o, n, a, i) {
    return e.name === "srcTexture" && n && i ? `srcTexture_${i(n.sourceId)}_${r}` : e.name !== "src" ? e.name : o && a ? `src_ext_${a(o.layerId)}_${r}` : `src_${r}`;
  }
  generateTransformCode(e, r, o, n, a, i, u, p, d, s, c, l, m, g, S) {
    const f = this.getFunctionName(r, s, l, m, g, S), y = (...h) => [...h, ...d].join(", ");
    let x = a, T = i, C = u, U = p;
    switch (r.type) {
      case "src": {
        const h = `c${o}`;
        e.push(`	vec4 ${h} = ${f}(${y(n)});`), x = h;
        break;
      }
      case "coord": {
        const h = `st${o}`;
        e.push(`	vec2 ${h} = ${f}(${y(n)});`), e.push(`	${n} = ${h};`);
        break;
      }
      case "color": {
        const h = `c${o}`;
        e.push(`	vec4 ${h} = ${f}(${y(a)});`), x = h;
        break;
      }
      case "combine": {
        const h = `c${o}`;
        e.push(`	vec4 ${h} = ${f}(${y(a, c ?? "vec4(0.0)")});`), x = h;
        break;
      }
      case "combineCoord": {
        const h = `st${o}`;
        e.push(`	vec2 ${h} = ${f}(${y(n, c ?? "vec4(0.0)")});`), e.push(`	${n} = ${h};`);
        break;
      }
    }
    return { colorVar: x, charVar: T, flagsVar: C, rotationVar: U };
  }
  _generateExternalSrcFunction(e, r, o) {
    const n = o(e.layerId), a = E(r);
    return `
...
`;
  ...
  _generateTextmodeSourceFunction(e, r, o) {
    const n = o(e.sourceId);
    return `
...
function ce(t) {
  ...
  if (Array.isArray(t)) {
    ...
    if (e >= 0) return `array with invalid element at index ${e}: ${ce(t[e])}`;
  }
  return String(t);
}
function Zt(t) {
...
function er(t, e, r) {
  return (o) => {
    ...
    try {
      ...
      return Z(a, e, o.onError), r;
    }
    return Zt(n) ? n : (Z(new Error(`[textmode.synth.js] Invalid dynamic parameter value for "${e}": ${ce(n)}`), e, o.onError), r);
  };
}
class tr {
  constructor() {
    __publicField(this, "_uniforms", /* @__PURE__ */ new Map());
    __publicField(this, "_dynamicUpdaters", /* @__PURE__ */ new Map());
  }
  processArgument(e, r, o) {
    return Gt(e) ? this._createDynamicUniform(r, o, (n) => Et(e, n)) : typeof e == "function" ? this._createDynamicUniform(r, o, e) : typeof e == "number" ? { glslValue: X(e) } : this.processDefault(r);
  }
  _createDynamicUniform(e, r, o) {
    const n = `${r}_${e.name}`, a = { name: n, type: e.type, value: e.default ?? 0, isDynamic: true }, i = er(o, n, a.value);
    return this._uniforms.set(n, a), this._dynamicUpdaters.set(n, i), { glslValue: n, uniform: a, updater: i };
  }
  processDefault(e) {
    const r = e.default;
...
`;
function or(t) {
  const { uniforms: e, glslFunctions: r, mainCode: o, charOutputCode: n, primaryColorVar: a, cellColorVar: i, charMapping: u, usesFeedback: p, usesCharFeedback: d, usesCellColorFeedback: s, usesCharSource: c, externalLayers: l, textmodeSources: m } = t, g = Array.from(e.values()).map((_) => `uniform ${_.type} ${_.name};`).join(`
`);
  let S = "", f = "";
...
uniform int u_charMapSize;`, f = `
  ...
  const y = [];
  p && y.push(`uniform sampler2D ${$.charColor};`), d && y.push(`uniform sampler2D ${$.char};`), s && y.push(`uniform sampler2D ${$.cellColor};`);
  const x = y.join(`
`), T = c ? "uniform float u_charSourceCount;" : "", C = [];
  if (l) for (const [, _] of l) _.usesChar && C.push(`uniform sampler2D ${_.uniformPrefix}${k.char};`), _.usesCharColor && C.push(`uniform sampler2D ${_.uniformPrefix}${k.charColor};`), _.usesCellColor && C.push(`uniform sampler2D ${_.uniformPrefix}${k.cellColor};`);
  const U = C.length > 0 ? `// External layer samplers
${C.join(`
`)}` : "", h = [];
  if (m) for (const [, _] of m) h.push(`uniform sampler2D ${_.uniformName};`), h.push(`uniform vec2 ${_.uniformName}_dim;`);
...
function j(t) {
  return new ar().compile(t);
}
class ar {
  constructor() {
    __publicField(this, "_uniformManager", new tr());
    __publicField(this, "_feedbackTracker", new Ht());
    __publicField(this, "_externalLayerManager", new Kt());
    __publicField(this, "_textmodeSourceManager", new Qt());
    __publicField(this, "_codeGenerator", new Wt());
    __publicField(this, "_glslFunctions", /* @__PURE__ */ new Set());
    __publicField(this, "_mainCode", []);
    __publicField(this, "_varCounter", 0);
    __publicField(this, "_currentTarget", "main");
    __publicField(this, "_usesCharSource", false);
  }
  compile(e) {
    this._reset();
    const r = this._compileChain(e, "main", "vec4(1.0, 1.0, 1.0, 1.0)", "v_uv", "main");
    let o = r.charVar;
    e.charSource && (o = this._compileCharSource(e));
    let n = r.colorVar;
    e.charColorSource && (n = this._compileChain(e.charColorSource, "charColor", "vec4(1.0, 1.0, 1.0, 1.0)", "v_uv", "charColor").colorVar);
    ...
    const i = nr(!!o, o ?? "vec4(0.0)", r.colorVar), u = this._feedbackTracker.getUsage();
    return { fragmentSource: or({ uniforms: this._uniformManager.getUniforms(), glslFunctions: this._glslFunctions, mainCode: this._mainCode, charOutputCode: i, primaryColorVar: n, cellColorVar: a, charMapping: e.charMapping, usesFeedback: u.usesCharColorFeedback, usesCharFeedback: u.usesCharFeedback, usesCellColorFeedback: u.usesCellColorFeedback, usesCharSource: this._usesCharSource, externalLayers: this._externalLayerManager.getExternalLayers(), textmodeSources: this._textmodeSourceManager.getSources() }), uniforms: this._uniformManager.getUniforms(), dynamicUpdaters: this._uniformManager.getDynamicUpdaters(), charMapping: e.charMapping, usesCharColorFeedback: u.usesCharColorFeedback, usesCharFeedback: u.usesCharFeedback, usesCellColorFeedback: u.usesCellColorFeedback, usesCharSource: this._usesCharSource, externalLayers: this._externalLayerManager.getExternalLayers(), textmodeSources: this._textmodeSourceManager.getSources() };
  }
  _reset() {
    this._varCounter = 0, this._uniformManager.clear(), this._feedbackTracker.reset(), this._externalLayerManager.reset(), this._textmodeSourceManager.reset(), this._glslFunctions.clear(), this._mainCode.length = 0, this._currentTarget = "main", this._usesCharSource = false;
  }
  _compileCharSource(e) {
    this._usesCharSource = true;
    const r = this._compileChain(e.charSource, "charSrc", "vec4(1.0, 1.0, 1.0, 1.0)", "v_uv", "char"), o = "charFromSource_" + this._varCounter++;
    return this._mainCode.push("	// Convert charSource color to character index"), this._mainCode.push(`	float charLum_${o} = clamp(_luminance(${r.colorVar}.rgb), 0.0, 1.0);`), this._mainCode.push(`	float charCount_${o} = max(u_charSourceCount, 1.0);`), this._mainCode.push(`	int charIdx_${o} = int(min(charLum_${o} * charCount_${o}, charCount_${o} - 1.0));`), this._mainCode.push(`	vec4 ${o} = _packChar(charIdx_${o});`), o;
  }
  _compileChain(e, r, o, n = "v_uv", a = "main") {
    const i = this._currentTarget;
    this._currentTarget = a;
    const u = `${r}_st`;
    let p, d, s, c = `${r}_c`;
    this._mainCode.push(`	vec2 ${u} = ${n};`), this._mainCode.push(`	vec4 ${c} = ${o};`);
    const l = e.transforms, m = l.map((f) => this._getProcessedTransform(f.name)), g = this._identifyCoordTransforms(m), S = (f) => {
      const y = l[f], x = m[f];
      if (!x) return void console.warn(`[SynthCompiler] Unknown transform: ${y.name}`);
      const T = e.externalLayerRefs.get(f), C = e.textmodeSourceRefs.get(f);
      y.name === "src" ? this._trackSrcUsage(T) : y.name === "srcTexture" && C && this._textmodeSourceManager.trackUsage(C, this._currentTarget);
      const U = this._codeGenerator.getContextAwareGlslFunction(x, y.name, this._currentTarget, T, C, (P) => this._externalLayerManager.getPrefix(P), (P) => this._textmodeSourceManager.getUniformName(P));
      this._glslFunctions.add(U);
      const h = this._processArguments(y.userArgs, x.inputs, `${r}_${f}_${y.name}`), _ = e.nestedSources.get(f);
      let K;
      _ && ee.has(x.type) && (K = this._compileChain(_, `${r}_nested_${f}`, o, u, a).colorVar);
      const F = this._codeGenerator.generateTransformCode(this._mainCode, x, this._varCounter++, u, c, p, d, s, h, this._currentTarget, K, T, C, (P) => this._externalLayerManager.getPrefix(P), (P) => this._textmodeSourceManager.getUniformName(P));
      c = F.colorVar, F.charVar && (p = F.charVar), F.flagsVar && (d = F.flagsVar), F.rotationVar && (s = F.rotationVar);
    };
    for (let f = 0; f < l.length; f++) m[f] && l[f].name === "seed" && S(f);
    for (let f = g.length - 1; f >= 0; f--) S(g[f]);
    for (let f = 0; f < l.length; f++) {
      const y = m[f];
      y && Q.has(y.type) || l[f].name !== "seed" && S(f);
    }
    return this._currentTarget = i, { coordVar: u, colorVar: c, charVar: p, flagsVar: d, rotationVar: s };
  }
  _identifyCoordTransforms(e) {
    const r = [];
    for (let o = 0; o < e.length; o++) {
      const n = e[o];
      n && Q.has(n.type) && r.push(o);
    }
    return r;
  }
  _trackSrcUsage(e) {
  ...
  _getProcessedTransform(e) {
    return R.getProcessed(e);
  }
  _processArguments(e, r, o) {
    const n = [];
    for (let a = 0; a < r.length; a++) {
      const i = r[a], u = e[a] ?? i.default, p = this._uniformManager.processArgument(u, i, o);
      n.push(p.glslValue);
    }
    return n;
  }
...
const _V = class _V {
  constructor() {
    __publicField(this, "_resolvedIndices");
    __publicField(this, "_lastFont");
    __publicField(this, "_lastChars", "");
  }
  resolve(e, r) {
    if (this._resolvedIndices && this._lastFont === r && this._lastChars === e) return this._resolvedIndices;
    let o = _V._fontIndexCache.get(r);
    if (!o) {
      o = /* @__PURE__ */ new Map();
      const d = r.characters;
      for (let s = 0; s < d.length; s++) o.set(d[s], s);
      _V._fontIndexCache.set(r, o);
    }
    const n = Array.from(e), a = new Int32Array(n.length), i = r.characterMap, u = i.get(" "), p = u && o.has(u) ? o.get(u) : 0;
    for (let d = 0; d < n.length; d++) {
      const s = n[d], c = i.get(s);
      if (c) {
        const l = o.get(c);
        a[d] = l !== void 0 ? l : p;
      } else a[d] = p;
    }
    return this._resolvedIndices = a, this._lastFont = r, this._lastChars = e, a;
  }
  invalidate() {
    this._resolvedIndices = void 0, this._lastFont = void 0, this._lastChars = "";
  }
};
__publicField(_V, "_fontIndexCache", /* @__PURE__ */ new WeakMap());
let V = _V;
function ie(t = {}) {
  return { source: t.source ?? new v(), sourceFactory: t.sourceFactory, compiled: t.compiled, shader: t.shader, characterResolver: t.characterResolver ?? new V(), needsCompile: t.needsCompile ?? false, isCompiling: t.isCompiling ?? false, pingPongBuffers: t.pingPongBuffers, pingPongDimensions: t.pingPongDimensions, pingPongIndex: t.pingPongIndex ?? 0, externalLayerMap: t.externalLayerMap, bpm: t.bpm, dynamicValues: t.dynamicValues ?? /* @__PURE__ */ new Map(), synthContext: t.synthContext ?? { time: 0, frameCount: 0, width: 0, height: 0, cols: 0, rows: 0, bpm: 0 }, isDisposed: false };
}
function sr(t) {
  t.extendLayer("synth", function(e) {
    const r = this.grid !== void 0 && this.drawFramebuffer !== void 0;
    let o, n;
    typeof e == "function" ? (n = e, o = new v()) : o = e;
    let a = this.getPluginState(b);
    a ? (a.source = o, a.sourceFactory = n, a.needsCompile = true, a.characterResolver.invalidate(), r && !n && (a.compiled = j(o))) : a = ie({ source: o, sourceFactory: n, compiled: r && !n ? j(o) : void 0, needsCompile: true }), this.setPluginState(b, a);
  });
}
function cr(t) {
  t.extendLayer("clearSynth", function() {
    ...
    e && (((_a = e.shader) == null ? void 0 : _a.dispose) && e.shader.dispose(), e.pingPongBuffers && ((_c = (_b = e.pingPongBuffers[0]).dispose) == null ? void 0 : _c.call(_b), (_e2 = (_d = e.pingPongBuffers[1]).dispose) == null ? void 0 : _e2.call(_d)), this.setPluginState(b, void 0));
  });
}
function ir(t) {
  t.extendLayer("bpm", function(e) {
    let r = this.getPluginState(b);
    r ? r.bpm = e : r = ie({ bpm: e }), this.setPluginState(b, r);
  });
}
const B = Symbol.for("textmode.synth.state");
function Y(t) {
  const e = t;
...
function I(t) {
  const e = /* @__PURE__ */ new Map();
  for (const [, r] of t.externalLayerRefs) {
    const o = typeof r.layer == "function" ? r.layer() : r.layer;
    o && e.set(r.layerId, o);
  }
  for (const [, r] of t.nestedSources) {
    const o = I(r);
    for (const [n, a] of o) e.set(n, a);
  }
  if (t.charSource) {
    const r = I(t.charSource);
    for (const [o, n] of r) e.set(o, n);
  }
...
function A(t) {
  const e = /* @__PURE__ */ new Map();
  for (const [, r] of t.textmodeSourceRefs) {
    const o = typeof r.source == "function" ? r.source() : r.source;
    o && e.set(r.sourceId, o);
  }
  for (const [, r] of t.nestedSources) {
    const o = A(r);
    for (const [n, a] of o) e.set(n, a);
...
class mr {
  constructor() {
    ...
    __publicField(this, "_isCompiling", false);
  ...
  async initialize(e) {
    if (!(this._shader || this._isCompiling || this._isDisposed)) {
      ...
      try {
        this._shader = await e.createFilterShader(dr);
      } catch (r) {
        console.warn("[textmode.synth.js] Failed to compile copy shader:", r);
      } finally {
  ...
  dispose() {
    var _a;
    this._isDisposed = true, ((_a = this._shader) == null ? void 0 : _a.dispose) && this._shader.dispose(), this._shader = null, this._isCompiling = false;
  }
  reset() {
...
async function hr(t, e) {
  ...
  if (!r) return;
  const o = t.grid, n = t.drawFramebuffer;
  if (!o || !n) return;
  let a = false;
  if (r.sourceFactory && r.needsCompile || !r.compiled) {
    let c = r.source, l = false;
    if (r.sourceFactory) try {
      const m = r.sourceFactory(), g = j(m);
      r.compiled && g.fragmentSource === r.compiled.fragmentSource || (r.source = m, c = m, r.compiled = g, a = false, l = true);
    } catch (m) {
      console.warn("[textmode.synth.js] Failed to evaluate synth factory:", m);
    }
    (l || !r.compiled || r.needsCompile) && (r.compiled || (r.compiled = j(c)), r.externalLayerMap = I(c), r.textmodeSourceMap = A(c), a = true, r.needsCompile = true);
  }
  if (r.needsCompile && r.compiled && !r.isCompiling) {
    r.isCompiling = true;
    const c = r.compiled;
    a || (r.externalLayerMap = I(r.source), r.textmodeSourceMap = A(r.source));
    try {
      const l = await e.createFilterShader(c.fragmentSource);
      if (r.isDisposed) return void (l.dispose && l.dispose());
      ((_a = r.shader) == null ? void 0 : _a.dispose) && r.shader.dispose(), r.shader = l, r.compiled === c && (r.needsCompile = false);
    } finally {
      r.isCompiling = false;
    }
  }
  if (!r.shader || !r.compiled || r.isDisposed) return;
  const i = r.compiled.usesCharColorFeedback, u = r.compiled.usesCharFeedback, p = r.compiled.usesCellColorFeedback, d = i || u || p;
  if (r.pingPongBuffers) {
    const c = r.pingPongDimensions, l = !c || c.cols !== o.cols || c.rows !== o.rows;
    d && !l || (r.pingPongBuffers[0].dispose(), r.pingPongBuffers[1].dispose(), r.pingPongBuffers = void 0, r.pingPongDimensions = void 0);
  }
  d && !r.pingPongBuffers && (r.pingPongBuffers = [e.createFramebuffer({ width: o.cols, height: o.rows, attachments: 3 }), e.createFramebuffer({ width: o.cols, height: o.rows, attachments: 3 })], r.pingPongDimensions = { cols: o.cols, rows: o.rows }, r.pingPongIndex = 0), r.synthContext || (r.synthContext = { time: 0, frameCount: 0, width: 0, height: 0, cols: 0, rows: 0, bpm: 0 });
  const s = r.synthContext;
  s.time = e.secs, s.frameCount = e.frameCount, s.width = o.width, s.height = o.height, s.cols = o.cols, s.rows = o.rows, s.bpm = r.bpm ?? lr(e), s.onError = r.onDynamicError, r.dynamicValues.clear();
  for (const [c, l] of r.compiled.dynamicUpdaters) {
    const m = l(s);
    r.dynamicValues.set(c, m);
  }
  if (d && r.pingPongBuffers) {
    const c = r.pingPongBuffers[r.pingPongIndex], l = r.pingPongBuffers[1 - r.pingPongIndex];
    l.begin(), e.clear(), e.shader(r.shader), O(t, e, r, s, c), e.rect(o.cols, o.rows), l.end(), n.begin(), e.clear();
    const m = D.getShader();
    m ? (e.shader(m), e.setUniform("u_charTex", l.textures[0]), e.setUniform("u_charColorTex", l.textures[1]), e.setUniform("u_cellColorTex", l.textures[2])) : (e.shader(r.shader), O(t, e, r, s, c)), e.rect(o.cols, o.rows), n.end(), r.pingPongIndex = 1 - r.pingPongIndex;
  } else n.begin(), e.clear(), e.shader(r.shader), O(t, e, r, s, null), e.rect(o.cols, o.rows), n.end();
  e.resetShader();
}
function O(t, e, r, o, n) {
  e.setUniform("time", o.time), e.setUniform("u_resolution", [o.cols, o.rows]);
  const a = ur(e);
  e.setUniform("u_seed", a ?? 0);
  for (const [s, c] of r.dynamicValues) e.setUniform(s, c);
  const i = r.compiled, u = r.staticUniformsAppliedTo !== r.shader;
  if (u) {
    for (const [s, c] of i.uniforms) c.isDynamic || typeof c.value == "function" || e.setUniform(s, c.value);
    r.staticUniformsAppliedTo = r.shader;
  }
  if (i.charMapping) {
    const s = r.characterResolver.resolve(i.charMapping.chars, t.font);
    (u || s !== r.lastCharMapIndices) && (e.setUniform("u_charMap", s), e.setUniform("u_charMapSize", s.length), r.lastCharMapIndices = s);
  }
  if (i.usesCharSource) {
    const s = i.charMapping ? i.charMapping.chars.length : t.font.characters.length;
    e.setUniform("u_charSourceCount", s);
  }
  n && (i.usesCharColorFeedback && e.setUniform($.charColor, n.textures[1]), i.usesCharFeedback && e.setUniform($.char, n.textures[0]), i.usesCellColorFeedback && e.setUniform($.cellColor, n.textures[2]));
  const p = i.externalLayers;
  if (p && p.size > 0 && r.externalLayerMap) for (const [s, c] of p) {
    const l = r.externalLayerMap.get(s);
    if (!l) {
      console.warn(`[textmode.synth.js] External layer not found: ${s}`);
      continue;
    }
    const m = l.getPluginState(b);
    let g;
    (m == null ? void 0 : m.pingPongBuffers) ? g = m.pingPongBuffers[m.pingPongIndex].textures : l.drawFramebuffer && (g = l.drawFramebuffer.textures), g && (c.usesChar && e.setUniform(`${c.uniformPrefix}${k.char}`, g[0]), c.usesCharColor && e.setUniform(`${c.uniformPrefix}${k.charColor}`, g[1]), c.usesCellColor && e.setUniform(`${c.uniformPrefix}${k.cellColor}`, g[2]));
  }
  const d = i.textmodeSources;
  if (d && d.size > 0 && r.textmodeSourceMap) for (const [s, c] of d) {
    const l = r.textmodeSourceMap.get(s);
    if (!l) {
      console.warn(`[textmode.synth.js] TextmodeSource not found: ${s}`);
      continue;
    }
    l.update && l.update();
    const m = l.width ?? 1, g = l.height ?? 1, S = m > 0 ? m : 1, f = g > 0 ? g : 1;
...
function yr(t) {
  ...
  e && (e.isDisposed = true, ((_a = e.shader) == null ? void 0 : _a.dispose) && e.shader.dispose(), e.pingPongBuffers && ((_c = (_b = e.pingPongBuffers[0]).dispose) == null ? void 0 : _c.call(_b), (_e2 = (_d = e.pingPongBuffers[1]).dispose) == null ? void 0 : _e2.call(_d)));
}
const gr = { name: b, version: "1.5.1", install(t, e) {
  D.reset(), fr(t), pr(t), sr(e), ir(e), cr(e), e.registerPreSetupHook(async () => {
    await D.initialize(t);
  }), e.registerLayerPreRenderHook((r) => hr(r, t)), e.registerLayerDisposedHook(yr);
}, uninstall(t, e) {
  var _a, _b, _c, _d, _e2;
  const r = [e.layerManager.base, ...e.layerManager.all];
  for (const o of r) {
    const n = o.getPluginState(b);
    n && (n.isDisposed = true, ((_a = n.shader) == null ? void 0 : _a.dispose) && n.shader.dispose(), n.pingPongBuffers && ((_c = (_b = n.pingPongBuffers[0]).dispose) == null ? void 0 : _c.call(_b), (_e2 = (_d = n.pingPongBuffers[1]).dispose) == null ? void 0 : _e2.call(_d)), o.setPluginState(b, void 0));
  }
...
function _r(t, e, r, o) {
  return new v({ cellColorSource: H(t, e, r, o) });
}
const vr = (t) => new v({ charSource: t });
function xr(t, e, r, o) {
  return new v({ charColorSource: H(t, e, r, o) });
}
function Cr(t) {
...
function Sr(t, e, r, o) {
  return w.plasma(t ?? null, e ?? null, r ?? null, o ?? null);
}
function wr(t, e, r, o, n, a) {
  return w.moire(t ?? null, e ?? null, r ?? null, o ?? null, n ?? null, a ?? null);
}
function $r(t, e, r) {
  return w.osc(t ?? null, e ?? null, r ?? null);
}
function Mr(t, e, r, o) {
  const n = H(t, e, r, o);
  return new v({ charColorSource: n, cellColorSource: n });
}
function Tr(t, e, r) {
  return w.shape(t ?? null, e ?? null, r ?? null);
}
function le(t, e, r, o) {
  return t !== void 0 && e === void 0 && r === void 0 && o === void 0 && typeof t == "number" ? w.solid(t) : w.solid(t ?? null, e ?? null, r ?? null, o ?? null);
}
const Fr = (t) => {
  const e = w.src;
  if (!t) return e();
  ...
  if (typeof t == "function") {
    const o = t();
    if (o && kr(o)) {
      const n = o.id ?? `layer_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;
      r.addExternalLayerRef({ layerId: n, layer: t });
    } else {
      const n = `tms_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;
      r.addTextmodeSourceRef({ sourceId: n, source: t });
    }
  } else if (Pr(t)) {
    const o = `tms_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;
    r.addTextmodeSourceRef({ sourceId: o, source: t });
  } else {
    const o = t, n = o.id ?? `layer_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;
    r.addExternalLayerRef({ layerId: n, layer: o });
  }
  return r;
};
function Pr(t) {
...
function Ir(t, e, r) {
  return w.voronoi(t ?? null, e ?? null, r ?? null);
}
function H(t, e, r, o) {
  return t instanceof v ? t : le(t, e, r, o);
}
typeof window < "u" && (window.SynthPlugin = gr, window.SynthSource = v, window.cellColor = _r, window.char = vr, window.charColor = xr, window.gradient = Cr, window.moire = wr, window.noise = br, window.osc = $r, window.paint = Mr, window.plasma = Sr, window.shape = Tr, window.solid = le, window.src = Fr, window.voronoi = Ir, window.setGlobalErrorCallback = Jt);
export {
  gr as SynthPlugin,
  v as SynthSource,
  _r as cellColor,
